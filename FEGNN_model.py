...
class BasisGenerator(nn.Module):
    ...
    '''
    def __init__(self, nx, nlx, nl, K, poly, low_x=False, low_lx=False, low_l=True, norm1=True):
...
class FEGNN(nn.Module):
    ...
    def __init__(self, args,ninput, nclass):
        ...
        self.K = args.K
        self.poly = args.poly
        self.nx = args.nhid if args.nx < 0 else args.nx # nx >= 0
        self.nlx = args.nhid if args.nlx < 0 else args.nlx # nlx >= 0
        self.nl = args.nl # nl 
        ...
        self.lin2 = nn.Linear(args.nhid, args.nhid, bias=True)
    ...
    def forward(self, x, edge_index, cp_adj,edge_weight):
        x, edge_index, cp_adj,edge_weight= x, edge_index, cp_adj,edge_weight
        ...
        if self.nx > 0:
            x_dict = F.relu(self.lin_x(x_basis))
        ...
        if self.nlx > 0:
            ...
            for k in range(self.K + 1):
                if self.share_lx:
                    ...
                    lx_b = F.relu(self.lin_lxs[k](lx_basis[k])) # do not share the W_lx parameters
                    
                lx_dict = lx_dict + lx_b
            dict_mat = dict_mat + lx_dict

        if self.nl > 0:
            l_dict = F.relu(self.lin_l(l_basis))
            dict_mat = dict_mat + l_dict


        res = F.relu(self.lin2(dict_mat))
