...
class LREvaluator(BaseEvaluator):
    ...
    def evaluate(self, x: torch.FloatTensor, y: torch.LongTensor, split: dict):
        ...
        return best_auc,best_aupr,best_accuracy,best_test_micro
...
class SC_CDG(torch.nn.Module):
    def __init__(self, encoder, augmentor, hidden_dim, proj_dim,data,weights=[0.50, 0.50, 0.50, 0.50]):
        super(SC_CDG, self).__init__()
        self.encoder = encoder
        self.augmentor = augmentor
        self.data=data
        self.fc1 = torch.nn.Linear(hidden_dim, proj_dim)
        self.fc2 = torch.nn.Linear(proj_dim, hidden_dim)
        self.linear1 = torch.nn.Linear(58, hidden_dim)
...
def train(encoder_model, contrast_model, data, optimizer,tr_mask, te_mask):
    encoder_model.train()
    optimizer.zero_grad()
    z, z1, z2 = encoder_model(data.x, data.edge_index, data,data.edge_attr)
    h1, h2 = [encoder_model.project(x) for x in [z1, z2]]
    loss_c = contrast_model(h1, h2)
    loss_m1 = F.binary_cross_entropy_with_logits(z[tr_mask], data.y[tr_mask].view(-1, 1))
...
def test(encoder_model, data):
    ...
    split = get_split(num_samples=z.size()[0], train_ratio=0.64, test_ratio=0.16)
    result = LREvaluator()(z, data.y, split)
    return result
@torch.no_grad()
def test1(encoder_model,data, mask):
    encoder_model.eval()
    z, _, _ = encoder_model(data.x, data.edge_index, data,data.edge_attr)
    pred = torch.sigmoid(z[mask])
...
def main():
    ...
    parser.add_argument('--nl', type=int, default=20, help='hidden size for the sturcture subdictionary, default 0 for not using this subdictionary') # chameleon 700, squirrel 2000
    ...
    parser.add_argument('--dataset_file', type=str, default='/home/dell/mxq/SC_CDG/data/GGNet/dataset_GGNet_ten_5CV.pkl',
    ...
    parser.add_argument('--ninput', type=int, default=58, help='Dimension of node features.')
    ...
    for i in range(10):
        for cv_run in range(5):
            ...
            fegnn=FEGNN(args,data.num_features,2)
            encoder_model = SC_CDG(encoder=fegnn, augmentor=(aug1, aug2), hidden_dim=args.nhid, proj_dim=args.nhid,data=data).to(device)
            contrast_model = DualBranchContrast(loss=InfoNCEBatched(tau=0.1, batch_size=1024), mode=args.mode, intraview_negs=False).to(device)
            optimizer = Adam(encoder_model.parameters(), lr=args.lr, weight_decay=args.w_decay)
            with tqdm(total=args.epochs, desc='(T)') as pbar:
                for epoch in range(1, args.epochs + 1):
                    # Training model
                    loss = train(encoder_model, contrast_model, data, optimizer,tr_mask, te_mask)
                    pbar.set_postfix({'loss': loss})
                    pbar.update()
                    if loss < best_loss:
                        best_loss = loss
                    ...
                    if current_tolerance == early_stopping_tolerance:
                        ...
                        break
                '''
                metrics.roc_auc_score(data.y[mask].cpu().numpy(), 
                pred.cpu().detach().numpy()), area, 
                data.y[mask].cpu().numpy(), 
                pred.cpu().detach().numpy()
                '''
                # AUC[i][cv_run], AUPR[i][cv_run], a, b = test(encoder_model, data)
                # # print(AUC[i][cv_run], AUPR[i][cv_run],a,b,'1')
                # a, b, all_y_true1, all_y_scores1 = test(encoder_model, data)
                AUC[i][cv_run], AUPR[i][cv_run], a, b = test1(encoder_model,data, te_mask)
                # print(AUC[i][cv_run], AUPR[i][cv_run],a,b,'1')
                a, b, all_y_true1, all_y_scores1 = test1(encoder_model,data, te_mask)
                # print(a,b,'2')
